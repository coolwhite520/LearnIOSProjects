...
- (void)initTableData
{
    self.subTitleArray = @[self.tip.uid, self.tip.name, self.tip.adcode, self.tip.district,
                           [NSString stringWithFormat:@"latitude:%f,longitude:%f",self.tip.location.latitude,self.tip.location.latitude]];
    
    self.titleArray = @[@"POI的id(uid)", @"名称(name)", @"区域编码(adcode)", @"所属区域(district)", @"位置(location)"];
}
